...
@Composable
fun FormularioCcarHechoDetenido(context: Context, navController: NavController) {
    ...
    var detenido by remember { mutableStateOf("") }
    ...
    FormConDetenido(
        ...
        detenido = DatosFormulario("Detenido/s", { detenido = it }, detenido),
...
fun FormConDetenido(
    ...
    Row(
        ...
        TextField(
            ...
            onValueChange = detenido.onChange,
            label = { Text(text = detenido.name) },
            singleLine = true,
            maxLines = 1,
            modifier = Modifier
                .width(400.dp)
...
private fun crearExtractoDetenido(
    ipp: DatosFormulario,
    caratula: DatosFormulario,
    jurisdiccion: DatosFormulario,
    denunciante: DatosFormulario,
    ...
    var detenidoVar = ""
    if (detenido.value != "") {
        detenidoVar = "\n\n*${detenido.name.uppercase().trim()}:* ${detenido.value.trim()}"
    }
    var intervencionVar = ""
    ...
    val extracto =
        fecha + encabezado + denuncianteVar + detenidoVar + intervencionVar + genesis + firma
