...
@Composable
fun FormularioCcarParadero(context: Context, navController: NavController) {
    ...
    var causante by remember { mutableStateOf("") }
    ...
    RowInputExtracto(
        ...
        causante = DatosFormulario("Causante/s", { causante = it }, causante),
...
fun RowInputExtracto(
    ...
    Row(
        ...
        TextField(
            value = causante.value,
            onValueChange = causante.onChange,
            label = { Text(text = causante.name) },
            singleLine = true,
            maxLines = 1,
            modifier = Modifier
                .width(400.dp)
    ...
    Row(
        ...
        FilledIconButton(
            ...
            Text(text = "Ccar Averiguación de Paradero")
        }
    }
...
private fun crearExtractoParadero(
    ipp: DatosFormulario,
    caratula: DatosFormulario,
    jurisdiccion: DatosFormulario,
    denunciante: DatosFormulario,
    ...
    var causanteVar = ""
    if (causante.value != "") {
        causanteVar = "\n\n*${causante.name.uppercase().trim()}:* ${causante.value.trim()}"
    }
    ...
    val extracto =
        fecha + encabezado + denuncianteVar + causanteVar + intervencionVar + genesis + firma
